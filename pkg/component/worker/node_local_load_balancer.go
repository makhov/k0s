...
type NodeLocalLoadBalancerComponent interface {
	component.Component
...
func NewNodeLocalLoadBalancer(cfg *constant.CfgVars, staticPods StaticPods) NodeLocalLoadBalancerComponent {
...
func (n *nodeLocalLoadBalancer) Stop() error {
	...
	case *nllbStopped:
		return nil
	default:
		return fmt.Errorf("cannot stop (%s)", state)
	}
}
...
func (n *nodeLocalLoadBalancer) start(state *nllbInitialized) error {
	started := &nllbStarted{state}

	if started.nllbPodSpec != nil {
		if err := n.provision(started); err != nil {
			return fmt.Errorf("node_local_load_balancer: cannot start: %w", err)
		}
	}
...
func (n *nodeLocalLoadBalancer) provision(state *nllbStarted) error {
